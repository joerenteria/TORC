...
unsigned long channel_1_fs = 1200;  //thro
unsigned long channel_2_fs = 1500;  //ail
unsigned long channel_3_fs = 1300;  //elev
unsigned long channel_4_fs = 1400;  //rudd
...
float Kd_roll_angle;      //Roll D-gain - angle mode (has no effect on controlANGLE2)
float B_loop_roll = 0.9;  //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)

float Kp_pitch_angle;      //Pitch P-gain - angle mode
float Ki_pitch_angle;      //Pitch I-gain - angle mode
float Kd_pitch_angle;      //Pitch D-gain - angle mode (has no effect on controlANGLE2)
...
int stopwatch = millis() - startTime;
bool cycleComp = false;
int zero = 0;
int stopPeriod = 1000;
int accPeriod = 2000;
int motStop = 188;
int startOffset = 0;
int startOffsetLimit = 12;
int minStart = 5;
int rampTime = 100;
bool rampComp = false;
...
void loop() {
  ...
  //printArmedFly();        //Prints armedFly boolean
  ...
  // Get arming status
  //Status(); //Check if the throttle cut is off and throttle is low.
  ...
  flightModePID();
  h(); //horizontal
  h2t(); //horizontal2transitional
  t2h(); //transitional2horizontal
  t(); //transitional
  t2v(); //transitional2vertical
  v2t(); //vertical2transitional
  v(); //vertical
  s(); //stopped
  motorDirection();
}
...
void controlMixer() {
  ...
  //horizontal
  //--------------------------------------------------
  if (flightMode == "h" || flightMode == "t2h") {
    m1_command_scaled = thro_des - yaw_PID;  //Front Left
    m2_command_scaled = thro_des + yaw_PID;  //Front Right
    m3_command_scaled = thro_des + yaw_PID;  //Back Right
    m4_command_scaled = thro_des - yaw_PID;  //Back Left
    m5_command_scaled = thro_des - yaw_PID;  //Booster
  }

  //--------------------------------------------------
  //transitional
  //--------------------------------------------------
  if (flightMode == "h2t" || flightMode == "t" || flightMode == "v2t") {
    m1_command_scaled = 0.75 * thro_des - pitch_PID + roll_PID;
    m2_command_scaled = 0.75 * thro_des - pitch_PID + roll_PID;
  ...
  //vertical
  //--------------------------------------------------
  if ((flightMode == "v2t" || flightMode == "t2v" || flightMode == "v") && cycleComp == true) {
    ...
    m5_command_scaled = thro_des - pitch_PID;
  }
}
...
//Texas (MK3)
//float Kp_roll_angle = 0.6;
//float Ki_roll_angle = 0.3;
//float Kd_roll_angle = 0.1;

void flightModePID() {
  //Horizontal
  if (flightMode == "t2h" || flightMode == "h") {
    Kp_pitch_angle = 0.0;
    Ki_pitch_angle = 0.0;
    Kd_pitch_angle = 0.00;
  ...
  //Transitional
  if (flightMode == "t" || flightMode == "h2t" || flightMode == "v2t") {
    Kp_pitch_angle = 0.2;
    Ki_pitch_angle = 0.3;
    Kd_pitch_angle = 0.05;
  ...
  //Vertical
  if (flightMode == "v2t" || flightMode == "v" || flightMode == "t2v") {
    Kp_pitch_angle = 0.4;
    Ki_pitch_angle = 0.3;
    Kd_pitch_angle = 0.075;
...
void motorDirection() {
  //motor1
  if (m1D == 1) {
    m1StartV = 188 + startOffset;
    m1StartH = 187 - startOffset;

    m1RunV = m1_command_scaled * 62 + 188;
    m1RunH = 187 - m1_command_scaled * 62;

    m1RunV = constrain(m1RunV, (188 + startOffsetLimit), 250);
    m1RunH = constrain(m1RunH, 125, (187 - startOffsetLimit));
  }

  if (m1D == 0) {
    m1StartH = 188 + startOffset;
    m1StartV = 187 - startOffset;

    m1RunH = m1_command_scaled * 62 + 188;
    m1RunV = 187 - m1_command_scaled * 62;

    m1RunH = constrain(m1RunH, (188 + startOffsetLimit), 250);
    m1RunV = constrain(m1RunV, 125, (187 - startOffsetLimit));
  }
  //motor2
  if (m2D == 1) {
    m2StartV = 188 + startOffset;
    m2StartH = 187 - startOffset;

    m2RunV = m2_command_scaled * 62 + 188;
    m2RunH = 187 - m2_command_scaled * 62;

    m2RunV = constrain(m2RunV, (188 + startOffsetLimit), 250);
    m2RunH = constrain(m2RunH, 125, (187 - startOffsetLimit));
  }

  if (m2D == 0) {
    m2StartH = 188 + startOffset;
    m2StartV = 187 - startOffset;

    m2RunH = m2_command_scaled * 62 + 188;
    m2RunV = 187 - m2_command_scaled * 62;

    m2RunH = constrain(m2RunH, (188 + startOffsetLimit), 250);
    m2RunV = constrain(m2RunV, 125, (187 - startOffsetLimit));
  }

  //motor3
  if (m3D == 1) {
    m3StartV = 188 + startOffset;
    m3StartH = 187 - startOffset;

    m3RunV = m3_command_scaled * 62 + 188;
    m3RunH = 187 - m3_command_scaled * 62;

    m3RunV = constrain(m3RunV, (188 + startOffsetLimit), 250);
    m3RunH = constrain(m3RunH, 125, (187 - startOffsetLimit));
  }

  if (m3D == 0) {
    m3StartH = 188 + startOffset;
    m3StartV = 187 - startOffset;

    m3RunH = m3_command_scaled * 62 + 188;
    m3RunV = 187 - m3_command_scaled * 62;

    m3RunH = constrain(m3RunH, (188 + startOffsetLimit), 250);
    m3RunV = constrain(m3RunV, 125, (187 - startOffsetLimit));
  }

  //motor4
  if (m4D == 1) {
    m4StartV = 188 + startOffset;
    m4StartH = 187 - startOffset;

    m4RunV = m4_command_scaled * 62 + 188;
    m4RunH = 187 - m4_command_scaled * 62;

    m4RunV = constrain(m4RunV, (188 + startOffsetLimit), 250);
    m4RunH = constrain(m4RunH, 125, (187 - startOffsetLimit));
  }

  if (m4D == 0) {
    m4StartH = 188 + startOffset;
    m4StartV = 187 - startOffset;

    m4RunH = m4_command_scaled * 62 + 188;
    m4RunV = 187 - m4_command_scaled * 62;

    m4RunH = constrain(m4RunH, (188 + startOffsetLimit), 250);
    m4RunV = constrain(m4RunV, 125, (187 - startOffsetLimit));
  }

    //booster
  if (m5D == 1) {
    m5StartV = 188 + startOffset;
    m5StartH = 187 - startOffset;

    m5RunV = m4_command_scaled * 62 + 188;
    m5RunH = 187 - m4_command_scaled * 62;

    m5RunV = constrain(boostRunV, (188 + startOffsetLimit), 250);
    m5RunH = constrain(boostRunH, 125, (187 - startOffsetLimit));
  }

  if (boostD == 0) {
    m5StartH = 188 + startOffset;
    m5StartV = 187 - startOffset;

    m5RunH = m4_command_scaled * 62 + 188;
    m5RunV = 187 - m4_command_scaled * 62;

    m5RunH = constrain(boostRunH, (188 + startOffsetLimit), 250);
    m5RunV = constrain(boostRunV, 125, (187 - startOffsetLimit));
  }
}
...
void flightModeSelect() {
  ...
  if (channel_6_pwm > 1800 && channel_6_pwm < 2000) {
    if (flightMode == "h2t" || flightMode == "t" || flightMode == "v2t" && cycleComp == true) {
      reset();
      flightMode = "t2h";
    }
    //fresh
    if (flightMode == "s" || flightMode == "v" || flightMode == "t2v" && cycleComp == true) {
      reset();
      flightMode = "h";
    }
  }
  ...
  if (channel_6_pwm > 1400 && channel_6_pwm < 1600) {
    if (flightMode == "h" || flightMode == "t2h" && cycleComp == true) {
      reset();
      flightMode = "h2t";
    }
    if (flightMode == "v" || flightMode == "t2v" && cycleComp == true) {
      reset();
      flightMode = "v2t";
    }
    //fresh
...
// Flight modes
//----------------------------------------------------------------------------------------------------
//Give or take 62... 125-187/188-250 (188 = stopped)
//--------------------------------------------------
// h
//--------------------------------------------------
...
void h() {
  if (flightMode == "h") {
    if (stopwatch > zero && stopwatch < stopPeriod) {
      m1_command_PWM = motStop;
      m2_command_PWM = motStop;
      m3_command_PWM = motStop;
      m4_command_PWM = motStop;
      m5_command_PWM = motStop;
    }

    if (stopwatch > stopPeriod && rampComp == false) {
      m1_command_PWM = m1StartH;
      m2_command_PWM = m2StartH;
      m3_command_PWM = m3StartH;
      m4_command_PWM = m4StartH;
      m5_command_PWM = boostStartH;

      startOffset = ((stopwatch - stopPeriod) / rampTime) + minStart;
    ...
    if (rampComp == true) {
      m1_command_PWM = m1RunH;
      m2_command_PWM = m2RunH;
      m3_command_PWM = m3RunH;
      m4_command_PWM = m4RunH;
      m5_command_PWM = boostRunH;
...
void h2t() {
  if (flightMode == "h2t") {
    if (stopwatch > zero && stopwatch < stopPeriod) {
      ...
      m3_command_PWM = motStop;
      m4_command_PWM = motStop;
      m5_command_PWM = m5RunH;
    }

    if (stopwatch > stopPeriod && rampComp == false) {
      m1_command_PWM = m1StartV;
      m2_command_PWM = m2StartV;
      m3_command_PWM = m3StartV;
      m4_command_PWM = m4StartV;
      m5_command_PWM = m5RunH;
    ...
    if (rampComp == true) {
      m1_command_PWM = m1RunV;
      m2_command_PWM = m2RunV;
      m3_command_PWM = m3RunV;
      m4_command_PWM = m4RunV;
      m5_command_PWM = m5RunH;
...
void v2t() {
  if (flightMode == "v2t") {
    ...
    if (stopwatch > accPeriod && stopwatch < (accPeriod + stopPeriod)) {
      m1_command_PWM = m1RunV;
      m2_command_PWM = m2RunV;
      m3_command_PWM = m3RunV;
      m4_command_PWM = m4RunV;
    ...
    if (stopwatch > (accPeriod + stopPeriod) && rampComp == false) {
      m1_command_PWM = m1RunV;
      m2_command_PWM = m2RunV;
      m3_command_PWM = m3RunV;
      m4_command_PWM = m4RunV;
      ...
      startOffset = ((stopwatch - (accPeriod + stopPeriod)) / rampTime) + minStart;
